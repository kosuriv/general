...
int fatfs_add_free_space(struct fatfs *fs, uint32 *startCluster, uint32 clusters)
{
    ...
    /* WDB Added*/
    static uint32 search_cluster = 0x00 ;

    if(search_cluster == 0x00)
    {
        search_cluster = fs->rootdir_first_cluster; 
    }
#ifndef BOOTLOADER
    printf("%s() Entry   search_cluster = %d Tick Count = %d \n",__FUNCTION__, search_cluster, xTaskGetTickCount());
#endif
    /* WDB */
    ...
    for (i=0;i<clusters;i++)
    {
        ...
        if (fatfs_find_blank_cluster(fs, search_cluster, &nextcluster))
        {
            // Point last to this
            ...
            search_cluster = nextcluster;   //WDB
        }
        else
        {
            nextcluster = 0x00;  //WDB 
            return 0;
        }
    }
