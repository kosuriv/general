...
return  NW_WIFI_ESP_ReceiveSocket(mqtt_socket_id,buffer, len,timeout_ms);
...
return NW_WIFI_ESP_SendSocket(mqtt_socket_id,buffer,len,timeout_ms);
...
#define FreeRTOS_disconnect  NetworkDisconnect
...
NW_WIFI_ESP_CloseSocket(mqtt_socket_id);
...
#ifdef ESP_WIFI
    if( port == 1885) // SSL port of MQTT
    {
        return NW_WIFI_ESP_OpenSocket(NW_WIFI_SSL_TCP_CLIENT,NW_ESP_WIFI_SOCKET_MQTT,addr, port,&mqtt_socket_id);
    }
    else
    {
        return NW_WIFI_ESP_OpenSocket(NW_WIFI_TCP_CLIENT,NW_ESP_WIFI_SOCKET_MQTT,addr, port,&mqtt_socket_id);
    }
#else
